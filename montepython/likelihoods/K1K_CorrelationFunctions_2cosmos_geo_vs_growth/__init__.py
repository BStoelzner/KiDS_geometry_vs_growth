####################################################################
# '2cosmos' likelihood for the KiDS-1000 cosmic shear correlation  #
...
# KiDS-1000 cosmic shear 2-point correlation function analysis from
# Asgari et al. 2020 (arXiv:2007.15633).
...
class K1K_CorrelationFunctions_2cosmos_geo_vs_growth(Likelihood):
    ...
    def load_data_file(self):
        ...
        xiP_estimator = data_tables['xiP'].data
        xiM_estimator = data_tables['xiM'].data
        data_vec = np.concatenate((np.asarray(xiP_estimator['VALUE']),np.asarray(xiM_estimator['VALUE'])))
    ...
    def get_theory_vec(self, Cls, Cl_keys, data, cosmo_index=1):
        ...
        input_theory['shear_cl'].update({'sample_a': 'shear_xi_plus'})
        input_theory['shear_cl'].update({'sample_b': 'shear_xi_minus'})
